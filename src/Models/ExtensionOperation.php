...
final class ExtensionOperation extends Model
{
    ...
    protected $table = 'gaminghub_extension_operations';
    ...
    public function fail(
        string $reason,
        string $category = 'extension_operation_failed',
        string $terminalStage = 'failed',
    ): void {
        ...
        if ($terminalStage !== 'failed') {
            $this->appendEvent($terminalStage, $terminalStage === 'rolled_back'
                ? 'Previous extension state restored.'
                : 'Automatic rollback failed.', $terminalStage === 'rolled_back' ? 'info' : 'error');
        }
        $this->save();
    }
}
