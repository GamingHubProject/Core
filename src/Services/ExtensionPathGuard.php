...
final class ExtensionPathGuard
{
    ...
    public function assertStagedDirectory(string $path, string $root, string $extensionId): void
    {
        ...
        if (! is_file($realPath.'/plugin.json') || ! is_file($realPath.'/gaming-hub-extension.json')) {
            throw new ExtensionOperationFailed('The staged extension package is missing required manifests.');
        }
    }
