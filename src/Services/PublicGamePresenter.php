...
final class PublicGamePresenter {
 ...
 private function make(Game $game,bool $include):PublicGameData{$server=$game->servers()->with('game')->enabled()->public()->ordered()->first();$status=$server?$this->servers->make($server):null;return new PublicGameData((int)$game->getKey(),$game->slug,$game->name,$game->short_description?:$game->description,$game->long_description?:$game->description,$game->icon_url,$game->banner_url,(int)$game->sort_order,$status?->status??'unknown',$status?->statusMessage,[]);}
}
