...
final class ExtensionUninstaller
{
    ...
    public function uninstall(
        ...
        $lock = Cache::lock('gaminghub:extension-operation:'.$extensionId, 180);
        $locked = false;
        $quarantine = null;
        $wasEnabled = false;
        $disabled = false;
        ...
        try {
            ...
            $operation->transition('disabling', 'Disabling the extension before file removal.');
            ...
            if ($exists) {
                $quarantine = $this->paths->pluginsRoot(true)
                    .'/.gaming-hub-uninstalling-'.$operation->operation_uuid;
            ...
            $operation->complete('Extension files removed. Extension database data was retained.');
