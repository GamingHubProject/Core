...
final class ProviderCreationTrace
{
    ...
    {
        return (bool) config('gaming-hub-core.providers.trace_creation', true);
    }
    ...
    private function safeMessage(string $message): string
    {
        ...
        $message = preg_replace(
            '/(token|password|secret|api[_-]?key|authorization)(\s*[=:]\s*)[^\s,;]+/i',
            '$1$2[redacted]',
            $message,
        ) ?? $message;
    ...
    private function sanitize(array $context): array
    {
        ...
        foreach ($context as $key => $value) {
            ...
            if (preg_match('/token|password|secret|api[_-]?key|authorization/', $normalized)) {
