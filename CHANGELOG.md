...
- Corrected active repository metadata to `https://github.com/GamingHubProject/Core` and added regression coverage against obsolete `RosesOfDorns` metadata.
- Validated Games as server-independent first-class domain objects; Game create/update continues to require no Server, Provider, or Panel fields.
- Kept Provider Instances Server-owned in current runtime while preserving deprecated Game/provider metadata non-destructively for compatibility.
- Established `short_description` and `long_description` as the current Game administration contract while retaining legacy `description` as a display fallback without destructive migration.
- Reworked Games administration to a responsive Grid default with an optional `?view=list` management view and safe fallback to Grid for unknown values.
- Added Game cards with artwork/fallback presentation, explicit enabled/disabled labels, informational Server counts, compact actions, ordering controls, and a first-Game empty state.
- Cleaned Create/Edit grouping into Identity, Presentation, and State without adding Server, Provider, Panel, or package-management fields.
- Added focused C0.2 regression and Blade/view contract coverage.
- No database schema changes.

## 0.7.10

- Finalized the C0.1 responsibility boundary without reintroducing package lifecycle or Panel ownership into Core.
- Kept Gaming Hub Manager integration optional and navigation-only; Core remains functional when Manager is absent or disabled.
- Preserved generic Provider bindings with no Panel-, Pelican-, or Pterodactyl-specific schema or credential fields.
- Disabled temporary provider-creation tracing by default; diagnostics now require explicit `GAMING_HUB_PROVIDER_TRACE=true` opt-in.
- Hardened provider trace redaction for token, API key, password, secret, authorization, and credential key variants, including casing/separator variations and sensitive assignments embedded in messages.
- Retained all historical package metadata migrations for non-destructive compatibility.
- Added focused C0.1 responsibility-boundary regression coverage.
- No database schema changes.
