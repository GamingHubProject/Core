...
## Upgrade from v0.7.10 to v0.7.11

1. Back up `plugins/gaming-hub-core`.
2. Replace only the `gaming-hub-core` plugin directory with v0.7.11.
3. Run `php artisan optimize:clear`.
4. No new migration is introduced by v0.7.11; historical package and Provider compatibility migrations must remain intact.
5. Open **Administration → Gaming Hub → Games** and verify Grid is the default; switch to List with the built-in view control.
6. Verify existing Games still render, including Games with zero Servers and Games that only have legacy `description` content.

## Upgrade from v0.7.0 to v0.7.10

1. Back up `plugins/gaming-hub-core`.
2. Replace only the `gaming-hub-core` plugin directory with v0.7.10.
3. Run `php artisan optimize:clear`.
4. No new migration is introduced by v0.7.10; keep all existing Core migrations, including the historical package metadata migrations.
5. Verify Games, Servers, Providers, public pages, and optional Gaming Hub Manager navigation.
6. Leave `GAMING_HUB_PROVIDER_TRACE` unset/false for normal production use; enable it only temporarily when sanitized provider-creation diagnostics are needed.
